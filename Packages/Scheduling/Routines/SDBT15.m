...
DE S DIE="^SC(",DIC=DIE,DP=44,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$N(^SC(DA,-1))<0
 ...
 I $D(^("SL")) S %Z=^("SL") S %=$P(%Z,U,5) S:%]"" DE(2)=% S %=$P(%Z,U,7) S:%]"" DE(3)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
2 S DW="SL;5",DV="*P44'",DU="",DLB="PRINCIPAL CLINIC",DIFLD=1916
 S DU="SC("
 G RE
X2 S DIC("S")="I $P(^(0),""^"",3)=""C""" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
