...
DE S DIE="^SC(",DIC=DIE,DP=44,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$N(^SC(DA,-1))<0
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,18) S:%]"" DE(3)=%
 I $D(^("SDP")) S %Z=^("SDP") S %=$P(%Z,U,4) S:%]"" DE(1)=%
 I $D(^("SL")) S %Z=^("SL") S %=$P(%Z,U,8) S:%]"" DE(2)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="SDBT12",DQ=1
1 S DW="SDP;4",DV="RNJ3,0",DU="",DLB="MAX # DAYS FOR AUTO-REBOOK",DIFLD=2005
 G RE
X1 K:+X'=X!(X>365)!(X<1)!(X?.E1"."1N.N) X
 Q
 ;
2 S DW="SL;8",DV="S",DU="",DLB="SCHEDULE ON HOLIDAYS?",DIFLD=1918.5
 S DU="Y:YES;"
 G RE
X2 Q
3 S DW="0;18",DV="*P40.7'",DU="",DLB="CREDIT STOP CODE",DIFLD=2503
 S DU="DIC(40.7,"
 G RE
X3 S DIC("S")="I $P(^(0),U,2)'=900&$S('$P(^(0),U,3):1,$P(^(0),U,3)>DT:1,1:0)" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
4 D:$D(DG)>9 F^DIE17 G ^SDBT13
