...
DE S DIE="^SC(",DIC=DIE,DP=44,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$N(^SC(DA,-1))<0
 I $D(^("SL")) S %Z=^("SL") S %=$P(%Z,U,7) S:%]"" DE(1)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="SDBT23",DQ=1
1 S DW="SL;7",DV="RNJ4,0",DU="",DLB="OVERBOOKS/DAY MAXIMUM",DIFLD=1918
 G RE
X1 K:+X'=X!(X>9999)!(X<0)!(X?.E1"."1N.N) X
 Q
 ;
2 S D=0 K DE(1) ;1910
 S DIFLD=1910,DGO="^SDBT24",DC="1^44.03A^SI^",DV="44.03F",DW="0;1",DOW="SPECIAL INSTRUCTIONS",DLB="Select "_DOW
 I $D(DSC(44.03))#2,$P(DSC(44.03),"I $D(^UTILITY(",1)="" X DSC(44.03) S D=$N(^(0)) G M2
 S D=$S($D(^SC(DA,"SI",0)):$P(^(0),U,3,4),1:$N(^(0)))
M2 I D>0 S DC=DC_D I $D(^SC(DA,"SI",+D,0)) S DE(2)=$P(^(0),U,1)
 G RE
R2 D DE
 G A
 ;
3 D:$D(DG)>9 F^DIE17 G ^SDBT25
