...
REQTYP(AXY,FRDT,TODT,STATUS,DIVL,TYPS)   ;RPC - DSIR RPT REQUESTS BY TYPE
 N VEER,TYPE,LORQ,LODT,II,XX,YY,FLDS,XI,ALLST,MDIV,DIV,DIVS,CLCHK,RQTR S CLCHK=0 K ^TMP("DSIR",$J)
 S STATUS=$TR($G(STATUS),"~",U),DIVL=$TR($G(DIVL),"~",U)
 S MDIV=$D(^XUSEC("DSIR MDIV",DUZ)) S DIVS=$G(DIVL)]"" I DIVS F II=1:1:$L(DIVL,U) S:$P(DIVL,U,II) DIVS($P(DIVL,U,II))=""
 S AXY=$NA(^TMP("DSIROIREQTYP",$J)),YY=0,TYPS=$D(TYPS)>1,ALLST=(STATUS="ALL") K ^TMP("DSIROIREQTYP",$J)
 I '$G(FRDT)!'$G(TODT) S VEER="Missing From or To date!" G ERROR
...
TYLOOP(XRF) ;
 ...
 ...;Changed the next line of code for updated address processing
 ...;S XXB=XX D GETS(19620.12,RQTR,".14;.13;.11;.21;.22;.23;.24;.25;.26;.27")
