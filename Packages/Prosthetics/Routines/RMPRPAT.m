RMPRPAT ;PHX/RFM/JLT-DISPLAY 2319 FIRST PAGE ;8/29/1994
 ;;3.0;PROSTHETICS;**29,62,155**;Feb 09, 1996;Build 4
 ;
 ; RVD - patch # 62 - sets RMPRNAM, RMPRSSN,RMPRDOB and RMPRSSNE
 ;
ASK ;Set common variables
 Q:$G(RMPRDFN)<1
 D HOME^%ZIS S DFN=RMPRDFN,RMPRBACK=1,RMPRFRST=1 K:$G(RFLG)=1 RMPRFRST
...
ASK1 ;CALL ROUTINE TO DISPLAY SCREEN SELECTI0N
 ...
 D ASK1^RMPRPAT1 K ANS I $G(RMPRFRST)=1 D HELP^RMPRPAT1
 D ^DIR
 K DIR,RMPRFRST
 I Y["^" G EXIT
...
END D ELIG^VADPT
 ...
 G:$D(RMPRBACK) QUES
 W @IOF G ASK1
WRI I $Y>(IOSL-6),'$D(RMPRQUES) D QUEST1 Q:$D(RMPREND1)
...
QUEST1 S RMPRQUES=1
 ...
 I $D(DTOUT)!($D(DUOUT)) S RMPREND1=1 Q
 W ! Q
