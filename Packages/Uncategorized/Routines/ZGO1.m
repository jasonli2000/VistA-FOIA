ZGO1 ; Save globals to ZWR files organized by FileMan ; 5/23/12 1:12pm
 ;---------------------------------------------------------------------------
...
SAVEALL(DIR) ; Save all globals to files in host DIR
 ...
 D FILES
 D GLOBALS(.%ZG)
 S G="" F  S G=$O(%ZG(G)) Q:G=""  D GLOBAL(G)
 Q
...
FILE(N)
 ...
 Q FILE
GLOBALS(TG)
 F G="^ROUTINE","^TMP","^UTILITY","^XUTL" K TG(G)
 Q
GLOBAL(G) ; Dump global G
 U $P W "DUMPING GLOBAL"_G_".....",!
 N IO S IO=$$OPENGBL(G)
...
OPEN(IO)
 ;U $P W "OPEN ",IO,!
 C IO O IO:(newversion) ;I '$T U $P W "Cannot open """_IO_""" for write!",!
 Q
CLOSE(IO)
 ;U $P W "CLOSE ",IO,!
 C IO
