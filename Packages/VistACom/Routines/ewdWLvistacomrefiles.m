 ;GT.M version of page refiles (VistACom application)
...
pre() ;
 ...
 s sessionArray("ewd_prePageScript")="FILELIST^C0EPTFL"
 ...
 s sessionArray("ewd_pageName")="refiles"
 ...
 s Error=$$startSession^%zewdPHP("refiles",.%KEY,.%CGIEVAR,.sessionArray,.filesArray)
...
body ;
 ...
 w "Ext.getCmp('tempFiles').destroy();"_""
 w "Ext.regModel('patientFile',{fields:['text']});"_""
 w "zewdSTStorerefiles30=new Ext.data.JsonStore({model:'patientFile'});"_""
 w "EWD.sencha.widget.zewdSTrefiles30=new Ext.List({id:""patientFile"",itemTpl:""{text}"",store:zewdSTStorerefiles30,listeners:{itemtap:function(view,index,item,e){if (EWD.sencha.listClicked) return;EWD.sencha.listClicked = true;var record = zewdSTStorerefiles30.getAt(index);EWD.sencha.cardPanelAction['file']={transition:'slide',id:''}"_""
 w ";var nvp='listItemNo='+(index+1);var page='file';var synch=true;EWD.ajax.getPage({page:page,synch:synch,nvp:nvp}"_""
 w ");}}});"_""
 w "EWD.sencha.addWidget('refiles','patientFile');"_""
 w "EWD.sencha.widget.zewdSTrefiles21=new Ext.Panel({id:""tempFiles"",layout:""card"",scroll:""vertical"",items:[EWD.sencha.widget.zewdSTrefiles30]}"_""
 w ");"_""
 w "Ext.getCmp('files').add(Ext.getCmp('tempFiles'));  EWD.sencha.filesCnt = 'Imported CCR/CCD Files ("_$$getSessionValue^%zewdAPI("ptflCnt",sessid)_")';  Ext.getCmp('botToolBar').setTitle(EWD.sencha.filesCnt);"_""
 d writeListData^%zewdST2("ptFileList",1,sessid)
 w "EWD.sencha.loadListData(zewdSTStorerefiles30,EWD.sencha.jsonData);"_""
 w "Ext.getCmp('files').add(Ext.getCmp('tempFiles'));Ext.getCmp('files').doLayout();"_""
 w "EWD.sencha.loadCardPanel('refiles','tempFiles');"_""
 w "</pre>"_$c(13,10)
