...
quickTips(nodeOID,attrValues,docOID,technology)
 i $$updateExtJS^ExtJS("Ext.QuickTips.init();",,,docOID)
...
modalWindow(nodeOID,attrValues,docOID,technology)
 ...
 d getAttribs^ExtJS("Window",.widgetAttribs)
 d setAttribs^ExtJS(.win,.widgetAttribs,.attrs)
...
subPanelLayout(nodeOID,attrValues,docOID,technology)
 ...
 s textOID=$$updateExtJS^ExtJS(,jsText,,docOID)
 ;
 d removeIntermediateNode^%zewdDOM(nodeOID)
 QUIT
 ;
getSubPanels(nodeOID,panelRefs,docOID)
 ...
 d getAttribs^ExtJS("Panel",.widgetAttribs)
 ;
...
subPanel(nodeOID,panelRef,widgetAttribs,docOID)
 ...
 d setAttribs^ExtJS(.panel,.widgetAttribs,.mainAttrs)
 s objectList(1)="var^panelObject^"_panelRef
 s textOID=$$updateExtJS^ExtJS("",,.objectList,docOID)
...
widget(nodeOID,docOID)
 ...
 s x="s extClass=$t("_widgetName_"^ExtJSData2)" x x
 i extClass="" d  i extClass="" QUIT ""
 . s x="s extClass=$t("_widgetName_"^ExtJSData3)" x x
 s extClass=$p(extClass,";;",2)
 ;s x="s extClass=$p($t("_widgetName_"^ExtJSData2),"";;"",2)"
 ;x x
 ...
 d setAttribs^ExtJS(.configOptions,.widgetAttribs,.mainAttrs)
 ;
 s childNo=""
...
getAttribs(widgetName,widgetAttribs)
 ...
 ;
 s version=2
 s x="s line=$t("_widgetName_"^ExtJSData2)" x x
 i line="" s version=3
 s x="f lineNo=1:1 s line=$t("_widgetName_"+lineNo^ExtJSData"_version_") q:line[""***END***""  d addAttrib^ExtJS("""_widgetName_""",line,.widgetAttribs)"
 x x
 QUIT
 ;
dataStore(nodeOID)
...
grid(nodeOID,docOID)
 ...
 . s preText=""
 . f lineNo=1:1 s line=$t(updateSession+lineNo^ExtJSCode) q:line["***END***"  d
 . . s preText=preText_$p(line,";;",2,200)_$c(13,10)
 . s textOID=$$updateExtJS^ExtJS(,,,docOID,preText)
 . s id=$g(gridAttrs("id"))
 ...
 . n attr,listenersOID,lOID,text
 . s listenersOID=$$addListenersTag(nodeOID)
 . s attr("name")="rowclick"
 . s attr("param1")="grid"
 . s attr("param2")="rowClicked"
 . s attr("param3")="e"
 . s text="var selectionModel=grid.getSelectionModel();var record = selectionModel.getSelected();var currentRow=EWD.ext.getSelectedGridRecord(record);"_gridAttrs("onrowselect")_"(currentRow,rowClicked);"
...
handler(nodeOID)
 ...
 s textOID=$$updateExtJS^ExtJS(func,,,docOID)
...
ewdForm(nodeOID,panelRef,widgetAttribs,docOID)
 ...
 . i action'="" d actionFragment(action,sbOID)
 ...
 . n id,ifOID,inputValue,lOID,lsOID,name,nextpage,targetID,text
 . ;
 ...
 . s attr("method")="writeTextArea^ExtJS"
 . s attr("param1")=name
 . s attr("param2")="#ewd_sessid"
 . s attr("type")="procedure"
 . s xOID=$$addElementToDOM^%zewdDOM("ewd:execute",parentOID,,.attr,,1)
 . d setAttribute^%zewdDOM("value","<?= document.getElementById('zewdTextarea"_name_"').firstChild.data ?>",rOID)
 ...
 . i technology="wl"!(technology="gtm") s attr("arrayname")="sessionArray"
